...
# Streamlit App Setup
st.title("Astrology Chat Data Processor - Aggregate Counts")
...
if raw_file and completed_file and astro_file:
    ...
    # Step 3: Process Events to Calculate Unique Users for Aggregate Counts
    class UniqueUsersProcessor:
        ...
        def process_chat_intake_requests(self):
            ...
            # Count unique users for each astrologer
            user_counts = intake_events.groupby(['astrologerId'])['user_id'].nunique().reset_index()
        ...
        def process_chat_accepted_events(self):
            ...
            accept_counts = accept_events.groupby(['user_id'])['clientId'].nunique().reset_index()
        ...
        def process_chat_completed_events(self):
            ...
            completed_counts = completed_events.groupby(['astrologerId'])['userId'].nunique().reset_index()
        ...
        def process_paid_chat_completed_events(self):
            ...
            paid_counts = paid_events.groupby(['astrologerId'])['userId'].nunique().reset_index()
        ...
        def merge_with_astro_data(self, final_data):
            ...
            columns = ['_id', 'name', 'type', 'chat_intake_requests', 'chat_accepted', 'chat_completed', 'paid_chats_completed']
    ...
    final_results = pd.merge(final_results, accepted_data, on=['_id'], how='outer')
    final_results = pd.merge(final_results, completed_data, on=['_id'], how='outer')
    final_results = pd.merge(final_results, paid_completed_data, on=['_id'], how='outer')

    # Merge with astro data and display final data
    merged_data = processor.merge_with_astro_data(final_results)
    
    # Display final output
    st.write("### Final Processed Data (Aggregate Counts)")
    st.dataframe(merged_data)

    # Option to download final data
    csv = merged_data.to_csv(index=False)
    st.download_button("Download Final Data as CSV", data=csv, file_name="combined_data_final_aggregate.csv", mime="text/csv")
else:
